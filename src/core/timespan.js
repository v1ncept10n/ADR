window.TimeSpan = class TimeSpan {
   ...
   */
  constructor(ms) {
    Guard.isNumber(ms, "'мс' должно быть числом");
    this._ms = ms;
  }
  ...
  toString() {
    if (this.years > 1e6) {
      return `${format(this.totalYears, 3, 0)} лет`;
    }
  ...
  toStringNoDecimals() {
    ...
    addCheckedComponent(this.years, "год");
    addCheckedComponent(this.days, "день");
    addCheckedComponent(this.hours, "час");
    addCheckedComponent(this.minutes, "минута");
    addCheckedComponent(this.seconds, "секунда");
  ...
  toStringShort(useHMS = true, isSpeedrun = false) {
    ...
    if (totalSeconds < 10) {
      return `${format(totalSeconds, 0, 3)} секунд`;
    }
    if (totalSeconds < 60) {
      return `${format(totalSeconds, 0, 2)} секунд`;
    }
    if (this.totalHours < 100 || (isSpeedrun && this.totalHours < 1000)) {
      ...
      if (this.totalMinutes < 60) {
        return `${format(this.totalMinutes, 0, 2)} минут`;
      }
      if (this.totalHours < 24) {
        return `${format(this.totalHours, 0, 2)} часов`;
      }
    }
    if (this.totalDays < 500) {
      return `${isSpeedrun ? this.totalDays.toFixed(2) : format(this.totalDays, 0, 2)} дней`;
    }
    return `${isSpeedrun ? this.totalYears.toFixed(3) : format(this.totalYears, 3, 2)} лет`;
